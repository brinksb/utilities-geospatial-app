...
BEGIN
    ...
    SELECT source INTO start_vertex FROM network_edges WHERE id = p_edge_id;
    ...
    -- We use cost as hop count by setting all edges to cost 1
    RETURN QUERY
    WITH reachable AS (
        SELECT
            dd.node,
        ...
        FROM pgr_drivingDistance(
            'SELECT id, source, target, 1 as cost FROM network_edges',
            start_vertex,
            p_hops,
            false  -- undirected
        ) dd
    ...
    edges_in_range AS (
        ...
        FROM network_edges e
        JOIN reachable r1 ON e.source = r1.node
        LEFT JOIN reachable r2 ON e.target = r2.node
    ...
    JOIN network_edges e ON e.id = eir.id
